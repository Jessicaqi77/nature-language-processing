...
# based on the cky algorithm , generate the cky table and back pointer table
def generateTable(sentences, start, grammar_dict):
    start_time = start
    # Create the CYK table and  a back bp table
    sentence_pre = sentences.strip().split()
    #before deal with sentence, change the word into lower case
    sentence = [word.lower() for word in sentence_pre]
    length = len(sentence)
    table = []
    bp_table = []
...
#given these two tables and the max probability rule index, track back the route 
def parse(sentences, start, grammar_dict) :
    ...
    if generateTable(sentences, start, grammar_dict) :
       ...
       if table[0][length]:
           ...
           #print "Log Probability: " + str(math.log10(max))
...
def parseFromTable(sentence, table, bp, start, end, root):
    if bp[start][end]:
        ...
        #Before make a string sentence, change to the orginal case situation to match the standard tree
        if end == 1:
            list = [sentence[end - 1].capitalize()]
        else:
            list = [sentence[end - 1]]
    ...
    for line in f1:
        ...
        for node in t.bottomup():
            if node.children == []:
                ...
                pro_dict[node.label.lower()].append([node.children[0].label.lower()])
            else:
                temp = str(node.children[0].label.lower()) + ' ' + str(node.children[1].label.lower())
                pro_dict[node.label.lower()].append([temp])
